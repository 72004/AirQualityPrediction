...
with col1:
    ...
    if st.button("🚀 Generate Forecast"):
        ...
        # ensure features exist; allow lag features to be included even if not in df (they will be constructed inside generator)
        ...
        # store timezone-aware if input had tz, otherwise keep naive
        st.session_state['forecast_df'] = forecast_df
        st.success("✅ Forecast generated!")

# -------------------------
# 🆕 Show Next 72h Details (button -> slicer -> table + line chart)
# -------------------------
with col1:
    show_details = False
    # Use a persistent flag in session_state so UI persists after interactions
    ...
    # small "Hide" button to close panel
    if st.session_state.get('show_72h_details', False):
        if st.button("Hide Next 72h Details"):
            st.session_state['show_72h_details'] = False

    show_details = st.session_state.get('show_72h_details', False)
...
if show_details:
    if 'forecast_df' not in st.session_state:
        ...
        # Slicer for Today / Tomorrow / Day After Tomorrow
        ...
        if filtered.empty:
            st.warning("⚠️ No forecast data available for the selected day. Make sure you generated the forecast and that it covers the day selected.")
        else:
            # Show table and allow download
            # formatted table
            display_df = filtered[['datetime', 'pred_aqi', 'AQI Category']].copy()
            display_df = display_df.rename(columns={'pred_aqi': 'AQI (pred)'})
            ...
            # Line chart for the selected day
            fig_day = go.Figure()
            ...
            # also plot actuals if available for overlapping times
            # join df actuals by datetime to show actuals if present
...
# -------------------------
# Main plot (Actual vs Forecast preview)
# -------------------------
with col2:
    fig = go.Figure()
    ...
    if 'aqi' in df.columns:
        ...
        fig.add_trace(go.Scatter(x=sample_actuals['datetime'], y=sample_actuals['aqi'],
                                 mode='lines', name='Actual AQI'))
    if 'forecast_df' in st.session_state:
        ff = st.session_state['forecast_df']
        fig.add_trace(go.Scatter(x=ff['datetime'], y=ff['pred_aqi'],
                                 mode='lines+markers', name='Forecast AQI'))
    fig.update_layout(title="📈 AQI: Actual vs Forecast", xaxis_title="Datetime", yaxis_title="AQI", height=520)
